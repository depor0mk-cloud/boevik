...
async def cmd_boevik(message: types.Message):
    ...
    text = (
        ...
        "• /устроиться — Заработать 150-300 золота для клана (только участники). КД: 12ч\n"
...
@router.message(F.text.regexp(r'(?i)^/устроить?ся'))
...
async def rename_factory(message: types.Message):
    ...
    args = message.text.split(maxsplit=3)
    if len(args) < 4:
        await message.answer("⚠️ Использование: <code>/переименовать завод [тип: оружейный/финансовый/оборонительный] [новое название]</code>")
        return
    ...
    clan = get_db_ref(f'clans/{clan_id}').get()
    if clan.get('leader_id') != uid:
        await message.answer("⚠️ Только лидер может переименовывать заводы.")
        return
        
    btype = '1' if ftype == 'оружейный' else '2' if ftype == 'финансовый' else '3'
    factory_names = clan.get('factory_names', {})
    factory_names[btype] = new_name
    
    get_db_ref(f'clans/{clan_id}').update({
        'factory_names': factory_names
    })
    
    await message.answer(f"✅ Завод ({ftype}) переименован в «{html.escape(new_name)}».")
